...
class Smarty_Internal_Method_RegisterDefaultTemplateHandler
{
    ...
    /**
     * get default content from template or configs resource handler
     *
        ...
        if (is_string($_return)) {
            ...
            if ($source->exists) {
                ...
                throw new SmartyException("Default handler: Unable to load " .
                                          ($source->isConfig ? 'configs' : 'template') .
            ...
            $source->exists = false;
            throw new SmartyException('Default handler: No ' . ($source->isConfig ? 'configs' : 'template') .
